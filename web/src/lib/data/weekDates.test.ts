...
import { addDays, isoDate, mondayOf, weekEnding } from "./weekDates";
...
describe("normalising an anchor", () => {
  it("takes any day of a week to that week's Sunday", () => {
    /* Every one of a week's seven dates names the SAME window, which is fine
     * for a value in `useState` and impossible for one that has to be a finite
     * set of routes. The Sunday rather than the Monday because the field above
     * it reads *Last day*, and the window genuinely ends on the Sunday. */
    for (const d of [
      "2026-08-24",
      "2026-08-25",
      "2026-08-26",
      ...
      expect(weekEnding(d)).toBe("2026-08-30");
    }
  });

  it("is idempotent, so navigating twice cannot drift", () => {
    expect(weekEnding(weekEnding("2026-08-26"))).toBe("2026-08-30");
  });
});
